    ...
    <div style={{ position: "relative" }}>
      {isGetMenu ? (
        <div>
          ...
          >
            <div className="japanese_R">未受け取りの注文があります (詳細)</div>
          </Alert>
          <CategoryBar
            category={["メイン", "ドリンク", "トッピング"]}
            onClick={function (category: CategoryProp): void {
          ...
          <Grid container>
            {menu
              .filter(
                (item: any) => item.category === categoryMode && item.isStatus
              )
              ...
              onNext={async (payment) => {
                ...
                if (payment === "paypay") {
                  const resData = await axios.post(
                    apiUrl + "paypay?orderId=" + orderId + "&url=" + hostUrl,
                    {
                      amount: totalPrice,
                  ...
                  const resData = await axios.post(
                    "ttp://172.22.26.217:4242/create-checkout-session",
                    {
                      orderData: orderData,
                      orderId: orderId,
                    }
